...
import { PageHeader, Card, Button, Modal, Input, LoadingState } from '../components/ui';
...
const AdminPlans: React.FC = () => {
  ...
  return (
    <div className="space-y-8 font-poppins text-slate-800 dark:text-slate-200 pb-20">
      <PageHeader
        title="Dynamic Plan & Permissions Management"
        subtitle="Enable or disable specific features per purchased course plan tier."
      />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Plans List Cards */}
        {plans.map((plan) => (
          <Card key={plan._id} className="p-6 flex flex-col justify-between hover:shadow-xl hover:shadow-primary/5 transition duration-300">
            ...
            <Button
              ...
              className="w-full mt-6 flex items-center justify-center gap-1.5"
            >
      ...
      {selectedPlan && (
        <Modal
          ...
          description="Update pricing and toggle access to premium features."
        >
          <form onSubmit={handleSave} className="space-y-4 text-xs font-semibold mt-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-slate-500 dark:text-slate-400">Price (INR)</label>
                <Input
                  type="number"
                  required
                  value={price}
              ...
              <div className="space-y-1">
                <label className="text-slate-500 dark:text-slate-400">Duration (Months)</label>
                <Input
                  type="number"
                  required
                  value={duration}
